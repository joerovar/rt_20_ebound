...
def plot_pax_profile(bd, al, lp, os, through, pathname=None, x_y_lbls=None, controlled_stops=None):
    ...
    ax2 = ax1.twinx()
    ax1.bar(x1, bd, w, label='ons', color='black')
    ax1.bar(x2, al, w, label='offs', color='gray')
    ax2.plot(x1, lp, label='load', color='gray')
    ax2.plot(x1, through, label='through', color='gray', linestyle='dotted')
    if controlled_stops:
    ...
    ax2.set_yticks(np.arange(0, np.max(lp) + 1, 4))
...
def get_pax_times_fast(pax_set, n_stops, include_rbt=False):
    ...
    for rep in pax_set:
        ...
        denied_wt = denied_df['wait_time'].tolist()
        denied_rate_per_rep.append(denied_pax / tot_pax)
        denied_wait_times += denied_wt
    ...
    denied_wait_time_median = np.mean(denied_wait_times)
    ...
    return wait_time_set, denied_rate, denied_wait_time_median, rbt_od_set, pc_pax_wt_0_2, pc_pax_wt_2_4, pc_pax_wt_4_inf
